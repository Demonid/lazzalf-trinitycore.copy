            ...
            plMover->GetSession() && plMover->GetSession()->GetSecurity() < SEC_GAMEMASTER) //!plMover->GetCharmerOrOwnerPlayerOrPlayerItself()->isGameMaster())
        {
            ...
            if (World::GetEnableMistiming() && (sync_time > GetMistimingDelta))
            {
                ...
                if (bMistimingModulo)
                {
                    ...
                    if (difftime_log > World::GetLogCheatDeltaTime())
                    {
                        ...
                        sLog.outCheat("AC2-%s Map %u, mistiming exception #%d, mistiming: %dms", plMover->GetName(), plMover->GetMapId(), plMover->m_anti_MistimingCount, sync_time);
                    }
                    #endif
            ...
            if (!curDest)
            {
                ...
                if (World::GetEnableAntiGravity() && no_fly_auras && no_swim_in_water && plMover->m_anti_JumpBaseZ != 0 && JumpHeight < plMover->m_anti_Last_VSpeed)
                {
                    ...
                    {
                        plMover->m_logcheat_time = cServerTime;
                        sLog.outCheat("AC2-%s Map %u, AntiGravity exception. JumpHeight = %f, Allowed Vertical Speed = %f",
                            plMover->GetName(), plMover->GetMapId(), JumpHeight, plMover->m_anti_Last_VSpeed);
                    }
                    #endif
                ...
                // multi jump checks
                if (World::GetEnableAntiMultiJump() && opcode == MSG_MOVE_JUMP)
                {
                    if (no_fly_auras && no_swim_water)
                    {
                        if (plMover->m_anti_JumpCount >= 1)
                        {
                            ...
                            {
                                plMover->m_logcheat_time = cServerTime;
                                sLog.outCheat("AC2-%s Map %u, multi jump  exception", plMover->GetName(), plMover->GetMapId());
                            }
                            #endif
                    ...
                    if (World::GetEnableAntiSpeedTele() && (real_delta > allowed_delta))
                    {
                        ...
                        if (difftime_log > World::GetLogCheatDeltaTime())
                        {
                            ...
                            if (real_delta < 4900.0f)
                                sLog.outCheat("AC2-%s Map %u Area %u, X:%f Y:%f Z:%f, speed exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plMover->GetName(), plMover->GetMapId(), 
                                plMover->GetPositionX(), plMover->GetPositionY(), plMover->GetPositionZ(), plMover->GetAreaId(), real_delta, allowed_delta, current_speed, plMover->m_anti_Last_HSpeed, time_delta);
                            else
                                sLog.outCheat("AC2-%s Map %u Area %u, X:%f Y:%f Z:%f, teleport exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plMover->GetName(), plMover->GetMapId(), 
                                plMover->GetPositionX(), plMover->GetPositionY(), plMover->GetPositionZ(), plMover->GetAreaId(), real_delta, allowed_delta, current_speed, plMover->m_anti_Last_HSpeed, time_delta);
                        }
                        #endif
                ...
                if (World::GetEnableAntiMountainHack() && (delta_z < plMover->m_anti_Last_VSpeed && plMover->m_anti_JumpCount == 0 && tg_z > 2.37f))
                {
                    ...
                    {
                        plMover->m_logcheat_time = cServerTime;
                        sLog.outCheat("AC2-%s Map %u, umountain exception | tg_z=%f", plMover->GetName(), plMover->GetMapId(), tg_z);
                    }
                    #endif
                ...
                if (!plMover->GetVehicle()) // Disable on Vehicle
                {
                    if (World::GetEnableAntiFlyHack() && no_fly_auras && !no_fly_flags)
                    {
                        ...
                        {
                            plMover->m_logcheat_time = cServerTime;
                            sLog.outCheat("AC2-%s Map %u Area %u X:%f Y:%f Z:%f flight exception. {SPELL_AURA_FLY=[%X]} {SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACK=[%X]} {plMover->GetVehicle()=[%X]}",
                ...
                if (World::GetEnableAntiWaterwalk() && no_waterwalk_auras && !no_waterwalk_flags)
                {
                    ...
                    {
                        plMover->m_logcheat_time = cServerTime;
                        sLog.outCheat("AC2-%s Map %u, waterwalk exception. {movementInfo=[%X]}{SPELL_AURA_WATER_WALK=[%X]}",
                            plMover->GetName(), plMover->GetMapId(), movementInfo.flags, plMover->HasAuraType(SPELL_AURA_WATER_WALK));
                    }
                    #endif
                ...
                if (World::GetEnableTeleportToPlane() && no_swim_in_water && movementInfo.pos.GetPositionZ() < 0.0001f && movementInfo.pos.GetPositionZ() > -0.0001f)
                {
