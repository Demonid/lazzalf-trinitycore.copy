...
void AuraEffect::TriggerSpell(Unit * target, Unit * caster) const
{
    ...
    else if (target->GetTypeId() != TYPEID_UNIT || (triggerTarget->ToCreature() && !sScriptMgr.OnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex()), triggerTarget->ToCreature())))
    {
...
void AuraEffect::HandleShapeshiftBoosts(Unit * target, bool apply) const
{
    ...
    switch(GetMiscValue())
    {
        case FORM_CAT:
            spellId = 3025;
            spellId2 = 21184; //Rogue Passive (DND)
            HotWSpellId = 24900;
            break;
        case FORM_TREE:
            spellId = 34123;
