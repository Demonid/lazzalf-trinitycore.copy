...
struct npc_sinclariAI : public ScriptedAI
{
	...
	{
		if (Creature* pSummon = me->SummonCreature(NPC_DEFENSE_SYSTEM, fDefenseSystemLoc[0], fDefenseSystemLoc[1], fDefenseSystemLoc[2], fDefenseSystemLoc[3], TEMPSUMMON_TIMED_DESPAWN, 10000))
		{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
		...
		{
			//me->MonsterYell("Containment Failed!", LANG_UNIVERSAL, 0);
			DoScriptText(EMOTE_SINCLARY_FAIL,me);
			yelled_failure=true;
		}
...
struct mob_vh_dragonsAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff){
		...
		if (creatureEntry != CREATURE_PORTAL_GUARDIAN &&  creatureEntry != CREATURE_PORTAL_KEEPER && m_ui_CorruptSeal_Timer <= uiDiff)
        {
			...
			if(pMainDoor && !me->isInCombat()){
				if(me->IsWithinDist(pMainDoor, 27.0f, false))
				{
					me->GetMotionMaster()->Clear();
					DoCast(me,SPELL_CORRUPT);
    ...
    void PortalKeeper_UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(me->getVictim(), m_bIsRegular ? SPELL_ARCANE_MISSILE : SPELL_ARCANE_MISSILE_H);
            m_uiArcaneMissile_Timer = 10000;
        ...
        {
            DoCast(me->getVictim(), SPELL_DEEP_FREEZE);
        ...
        {
            DoCast(me->getVictim(), m_bIsRegular ? SPELL_FROSTBOLT_VOLLEY : SPELL_FROSTBOLT_VOLLEY_H);
            m_uiFrostBoltVolley_Timer = 15000;
    ...
    void PortalGuardian_UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(me->getVictim(), SPELL_AGONIZING_STRIKE);
            m_uiAgonizingStrike_Timer = 10000;
        ...
        {
            DoCast(me->getVictim(), SPELL_SIDE_SWIPE);
            m_uiSideWipe_Timer = 8000;
    ...
    void AzureSorceror_UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(me, m_bIsRegular ? SPELL_MANA_DETONATION : SPELL_MANA_DETONATION_H);
    ...
    void AzureStalker_UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(me->getVictim(), SPELL_TACTICAL_BLINK);
    ...
    void AzureInvader_UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(me->getVictim(), m_bIsRegular ? SPELL_IMPALE : SPELL_IMPALE_H);
	...
	void AzureBinder_UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(me->getVictim(), m_bIsRegular ? SPELL_ARCANE_BARRAGE : SPELL_ARCANE_BARRAGE_H);
            m_uiArcaneBarrage_Timer = 8000;
        ...
        {
            DoCast(me, m_bIsRegular ? SPELL_ARCANE_EXPLOSION : SPELL_ARCANE_EXPLOSION_H);
	...
	void AzureSpellBreaker_UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(me, m_bIsRegular ? SPELL_ARCANE_BLAST : SPELL_ARCANE_BLAST_H);
            m_uiArcaneBlast_Timer = 7000;
        ...
        if (uiSpawnTimer <= diff)
        {
            if (bPortalGuardianOrKeeperSpawn)
            {
                ...
                for (uint8 i = 0; i < k; ++i)
                {
                    ...
                    if (Creature* pSummon = DoSummon(entry, me, 2.0f, 30000, TEMPSUMMON_DEAD_DESPAWN/*TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT*/))
    ...
    void JustDied(Unit* killer)
    {
        if (pInstance)
        {
            ...
            if(pInstance->GetData(DATA_WAVE_COUNT) == 6 || pInstance->GetData(DATA_WAVE_COUNT) == 12)
                Creature* pSummon = DoSummon(CREATURE_AZURE_SABOTEUR, me, 2.0f, 30000, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT);
        }
    }
};
    ...
    void UpdateAI(const uint32 uiDiff)
    {
		if (m_pInstance && !started)
        {
			...
			if(m_uiBossID > 0){
				...
				me->GetMotionMaster()->MovePoint(0, BossLoc[m_uiBossID].x,  BossLoc[m_uiBossID].y,  BossLoc[m_uiBossID].z);
			}
        ...
        if(dest_Door){
			if(me->IsWithinDist(dest_Door, 27.0f, false))
				{
					me->GetMotionMaster()->Clear();
					DoCast(me, SPELL_SHIELD_DISRUPTION);
