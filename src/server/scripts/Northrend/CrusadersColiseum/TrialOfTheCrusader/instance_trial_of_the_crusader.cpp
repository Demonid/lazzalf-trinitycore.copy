...
{
    instance_trial_of_the_crusader(Map* pMap) : ScriptedInstance(pMap), m_uiChampionGest(NULL), Floor(NULL) {Initialize();}
    ...
    GameObject* Floor, *m_uiChampionGest;
    std::string str_data;
  ...
  switch(pCreature->GetEntry())
	...
	pCreature->UpdateEntry(HIGHLORD_TIRION,HORDE);
	break;
    case NPC_KRIK:
	...
	case GO_CHAMPIONS_CHEST:
	case GO_CHAMPIONS_CHEST_H:
//	case GO_HOLY_CHEST:
//	case GO_HOLY_CHEST_H:
//	case GO_SHADOW_CHEST:
//	case GO_SHADOW_CHEST_H:
	m_uiChampionGest = add ? pGo : NULL; break;
        }
    }
	...
	case TYPE_CHAMPIONS: Encounters[3] = uiData;
		if(uiData == DONE)
		{
		  ...
		  {
		  pAnnouncer->SummonGameObject(instance->IsHeroic()? GO_CHAMPIONS_CHEST_H : GO_CHAMPIONS_CHEST,563.58, 139.64, 393.83,1.42,0, 0, 0, 0,90000000);
	...
	case DATA_BOSS_ANUBARAK: Encounters[5] = uiData;
	    if(uiData == DONE)
		{
		  if (Creature* pAnnouncer =  instance->GetCreature(m_uiAnnouncer))
		  {
		 // pAnnouncer->SummonGameObject(instance->IsHeroic()? GO_SHADOW_CHEST_H : GO_SHADOW_CHEST,643.44,144.05,140.93,3.42,0, 0, 0, 0,90000000);
